// Script para generar el logo real de Barna Gestoría como fallback
export const generateBarnaLogoSVG = () => {
  const logoSVG = `
    <svg width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <linearGradient id="barnaGradient" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" style="stop-color:#2D5A3D;stop-opacity:1" />
          <stop offset="100%" style="stop-color:#1F4A2A;stop-opacity:1" />
        </linearGradient>
      </defs>
      
      <!-- Forma principal del escudo/círculo -->
      <path d="M20 2 C30 2, 36 8, 36 18 C36 28, 30 38, 20 38 C10 38, 4 28, 4 18 C4 8, 10 2, 20 2 Z" 
            fill="url(#barnaGradient)" stroke="#1F4A2A" strokeWidth="1"/>
...
// Función para crear el logo real como imagen en canvas
export const createBarnaLogoImage = () => {
  ...
  // Fondo del escudo con gradiente verde oscuro
  const gradient = ctx.createLinearGradient(0, 0, 40, 40)
  gradient.addColorStop(0, "#2D5A3D")
  gradient.addColorStop(1, "#1F4A2A")

  // Forma del escudo/círculo
  ctx.fillStyle = gradient
  ctx.beginPath()
  ctx.moveTo(20, 2)
  ctx.bezierCurveTo(30, 2, 36, 8, 36, 18)
  ctx.bezierCurveTo(36, 28, 30, 38, 20, 38)
  ctx.bezierCurveTo(10, 38, 4, 28, 4, 18)
  ctx.bezierCurveTo(4, 8, 10, 2, 20, 2)
  ctx.closePath()
  ctx.fill()
...
// Función para generar un logo simplificado usando solo CSS/HTML
export const generateCSSLogo = () => {
  return `
    <div style="
      width: 40px;
      height: 40px;
